    ...
    foreach ($itemRaw->fields as $itemRawField) {
      ...
      foreach ($jsonView as $viewKey => $viewConfig) {
        ...
        if (in_array($itemRawField->external_id, $viewConfig['fields'])) {
          $podioViewData[$viewKey][$itemRaw->item_id]['fields'][$itemRawField->external_id] = array(
            #'type' => $itemRawField->type,
            #'raw' => '',
            #'value' => '',
          );
          foreach ($itemRawField->values as $num => $itemRawFieldValue) {
            switch ($itemRawField->type) {
            	case 'text':
            	case 'state':
            	  $podioViewData[$viewKey][$itemRaw->item_id]['fields'][$itemRawField->external_id][$num] = array(
          ...
          switch ($itemRawField->type) {
            case 'text':
            case 'state':
                $podioViewData[$viewKey][$itemRaw->item_id]['fields'][$itemRawField->external_id]['raw'] = htmlspecialchars($itemRawField->values[0]->value);
                $podioViewData[$viewKey][$itemRaw->item_id]['fields'][$itemRawField->external_id]['value'] = htmlspecialchars($itemRawField->values[0]->value);
              break;
          }
          */
        }
  ...
  foreach ($podioViewData[$viewKey] as $itemId => $itemData) {
    ...
    foreach ($viewConfig['fields'] as $fieldExternalId) {
      if (!isset($podioViewData[$viewKey][$itemId]['fields'][$fieldExternalId])) {
        $podioViewData[$viewKey][$itemId]['fields'][$fieldExternalId] = array();
      }
    }
...
#print "COUNT VIEW: ".count($podioViewData[1])."\n";
#print "COUNT VIEW: ".count($podioViewData[2])."\n";
#print_r($jsonViewData);
#print_r(array_shift($podioItemRawData));